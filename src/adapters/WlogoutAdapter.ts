...
const REQUIRED_TOKENS = ['primary', 'foreground'];

export interface WlogoutAdapterConfig {
    outputPath:       string;
...
export class WlogoutAdapter {
    public async apply(spore: Spore, config: WlogoutAdapterConfig): Promise<Report> {
        const startedAt = new Date().toISOString();

        const missing = REQUIRED_TOKENS.filter(k => spore.colors[k] === undefined);
        if (missing.length > 0) {
            throw new Error(`Recipe "${spore.recipe}" is missing wlogout tokens: ${missing.join(', ')}`);
        }
