...
Config (source definitions in ~/.config/unclaimed-bloom/):
  ...
  profile  [list | show <name> | current | use <name> | validate <name>]
...
Global flags:
  ...
  private async printStatus(): Promise<void> {
    ...
    const currentProfile = await this.readCurrentProfile(paths);
    const lastProfile = [plantState, growState, sowState].find((s) => s?.profile)?.profile;

    if (this.outputMode === "json") {
      this.printJson({
        ok: true,
        status: { ...pathSummary, currentProfile, sow: sowState, grow: growState, plant: plantState },
      });
      return;
    }
    ...
    if (currentProfile) {
      infoFields.push({ label: "current", value: currentProfile });
    }
...
For config definitions use: palette show, profile show, recipe show, mood show.`);
  ...
  private async runProfileCommand(args: string[]): Promise<void> {
    ...
    if (subcommand === "use" || subcommand === "set-current") {
      await this.useProfile(args[1]);
      return;
    }
  ...
  private async showCurrentProfile(): Promise<void> {
    const paths = BloomPaths.fromDeno();
    const profile = await this.readCurrentProfile(paths) ?? "desktop";
    ...
    if (this.outputMode === "json") {
      this.printJson({
        ...
        profile,
        path: paths.currentProfileFile(),
      });
      return;
    }

    this.printHuman(profile);
  }
  ...
  private async useProfile(name: string | undefined): Promise<void> {
    ...
    this.printHuman(`Current wallpaper profile: ${name}`);
    this.printHuman(this.display.dim(`  ${paths.currentProfileFile()}`));
  }

  private async readCurrentProfile(paths: BloomPaths): Promise<string | null> {
    try {
      const profile = (await Deno.readTextFile(paths.currentProfileFile())).trim();
